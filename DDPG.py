# -*- coding: utf-8 -*-
"""
Last update on Wed Apr 27 16:15:41 2022
...
###############################            
class Buffer:
    def __init__(self,actor_model,critic_model,target_actor,target_critic,actor_optimizer,critic_optimizer, buffer_capacity=100000, batch_size=64):
    ...
    def update(
        ...
        with tf.GradientTape() as tape:
            ...
            tc =  self.target_critic([next_state_batch, target_actions], training=True)
...
#Buffer per each ASV
buffer1 = Buffer(actor_model1,critic_model1,target_actor1,target_critic1,actor_optimizer1,critic_optimizer1,100000, 64)
buffer2 = Buffer(actor_model2,critic_model2,target_actor2,target_critic2,actor_optimizer2,critic_optimizer2,100000, 64)
##############################
...
for ep in range(total_episodes):
    ...
    while True:
        ...
        buffer1.record((prev_state[0], action1, reward[0], state[0]))
        buffer2.record((prev_state[1], action2, reward[1], state[1]))
    ...
    #In case the average reward is higher than -200, the models are stored
    if avg_reward1>-200 and avg_reward2>-200 and n_eps>50:
                timenow = dtime.now()
                tstamp = timenow.strftime('_%d_%m_%M_%S')
                filename1 = 'Mod_ddpg1''_'+str(ep)+tstamp+'.h5'
                actor_model1.save(filename1)
                filename2 = 'Mod_ddpg2''_'+str(ep)+tstamp+'.h5'
    ...
    if ep>(total_episodes-100) and episodic_reward1>-200 and episodic_reward2>-200:
        timenow = dtime.now()
        tstamp = timenow.strftime('_%d_%m_%M_%S')
        filename1 = 'Mod_ddpg1''_'+str(ep)+tstamp+'.h5'
        actor_model1.save(filename1)
...
filename1 = 'Model_ddpg1.h5'
actor_model1.save(filename1)  
filename2 = 'Model_ddpg2.h5'
actor_model2.save(filename2)      
